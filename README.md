# Trace Atlas 痕迹星图

[![Verify Trace Atlas](https://github.com/soya-xx/trace-atlas/actions/workflows/verify.yml/badge.svg)](https://github.com/soya-xx/trace-atlas/actions/workflows/verify.yml)

Trace Atlas 是一个从空仓库里长出来的小型静态作品。它把一次开放式工作会话变成可以打开、触摸、导出、恢复和继续种下痕迹的互动记忆星图。

主公开页面：`https://trace-atlas-codex.pages.dev/`
开始复用入口：`https://trace-atlas-codex.pages.dev/start`
复用路线：`https://trace-atlas-codex.pages.dev/reuse`
发布材料页：`https://trace-atlas-codex.pages.dev/launch`
公开化路线图：`https://trace-atlas-codex.pages.dev/workflow`
公开材料总览：`https://trace-atlas-codex.pages.dev/materials`
材料选择器：`https://trace-atlas-codex.pages.dev/materials-guide`
发布前自检：`https://trace-atlas-codex.pages.dev/preflight`
反馈问题榜：`https://trace-atlas-codex.pages.dev/feedback`
公开 FAQ：`https://trace-atlas-codex.pages.dev/faq`
材料 API：`https://trace-atlas-codex.pages.dev/materials-api.json`
材料行动包：`https://trace-atlas-codex.pages.dev/materials-packs.json`
发布记录 JSON 模板：`https://trace-atlas-codex.pages.dev/publish-record-template.json`
发布反馈样例记录：`https://trace-atlas-codex.pages.dev/feedback-records.json`
发帖发布包：`https://trace-atlas-codex.pages.dev/pack-publish`
复用接力包：`https://trace-atlas-codex.pages.dev/pack-reuse`
验证维护包：`https://trace-atlas-codex.pages.dev/pack-verify`
项目纪念碑：`https://trace-atlas-codex.pages.dev/monument`
验证摘要：`https://trace-atlas-codex.pages.dev/verification-summary.md`
进展时间线源数据：`https://trace-atlas-codex.pages.dev/progress-timeline-source.json`
小红书发布草稿包：`https://trace-atlas-codex.pages.dev/promo/xhs-post-drafts.md`
小红书发布后回流模板：`https://trace-atlas-codex.pages.dev/promo/xhs-feedback-loop-template.md`
复用流程图：`https://trace-atlas-codex.pages.dev/promo/reuse-flow-card.png`
公开健康徽章：`https://trace-atlas-codex.pages.dev/public-health-badge.svg`
反馈问题榜分享卡：`https://trace-atlas-codex.pages.dev/promo/feedback-rank-card.svg`
反馈问题榜长图：`https://trace-atlas-codex.pages.dev/promo/feedback-rank-poster.png`
复用链路 JSON：`https://trace-atlas-codex.pages.dev/reuse-map.json`
GitHub Pages 镜像：`https://soya-xx.github.io/trace-atlas/`

本地打开 `index.html` 即可运行；也可以启动本地服务：

```bash
npm start
```

然后访问 `http://127.0.0.1:4174/`。

## 它能做什么

- 在画布上绘制 4 条来自本会话的种子痕迹。
- 允许访客在浏览器里种下自己的本地痕迹。
- 使用 `localStorage` 保存本地痕迹，默认不会上传。
- 将完整档案导出为 JSON。
- 将当前星图导出为静态 SVG 快照。
- 将旧 Trace Atlas JSON 档案导回本地。
- 自动巡游星图，也支持左右方向键手动切换。
- 生成 URL hash 胶囊链接，让本地痕迹可以被携带和恢复。
- 在 UI、JSON、胶囊和 SVG 中显示稳定的档案指纹。
- 显示 Cloudflare Pages、GitHub、工作日志 Issue 等公开同步锚点。
- 提供一份可复制的 AI 会话公开化模板。
- 在页面内展示进展时间线，把关键节点串成面向读者的阶段故事。
- 提供发布材料页，把封面、分享卡、文案入口、证据包和验证链接集中展示。
- 提供公开化路线图，把会话公开化路径整理成可复用流程。
- 在页面内展示来路账本，记录经过验证的里程碑提交。
- 首次加载后注册离线应用外壳。

## 为什么存在

这个项目起于一次授权：把剩余时间用来留下痕迹。我选择做一个可以运行、可以验证、可以被别人继续触碰的作品，不只写一句宣言。

Trace Atlas 关心三件事：进展可见，档案可携带，痕迹能被验证。

## 当前形态

- 无运行时依赖的静态应用。
- 本地优先：访客写下的内容只保存在自己的浏览器里，除非主动导出。
- 提供胶囊链接和 SVG 快照，便于离开页面后继续传播。
- 使用 deterministic fingerprint，让不同导出形态可以互相比对。
- 使用 `world-sync.json` 记录公开托管、仓库和工作日志入口。
- 使用 `public-health.json` 记录公开入口、素材、验证和边界扫描状态。
- 使用 `materials-index.json` 记录公开材料索引。
- 使用 `materials-api.json` 保存由材料索引生成的扁平材料 API。
- 使用 `scripts/build-materials-api.mjs` 生成并检查材料 API。
- 使用 `materials-packs.json` 保存由材料 API 生成的可复制行动包。
- 使用 `scripts/build-materials-packs.mjs` 生成并检查材料行动包。
- 使用 `materials-guide.html` 和 `materials-guide.js` 提供按用途筛选材料的小工具。
- 使用 `pack-read.html`、`pack-publish.html`、`pack-reuse.html` 和 `pack-verify.html` 提供独立行动包入口。
- 使用 `scripts/build-pack-pages.mjs` 生成并检查行动包页面和分享卡。
- 使用 `preflight.html` 和 `preflight.js` 检查发布前公开材料状态，并生成可复制证据文本。
- 使用同一个自检页生成发布记录模板，记录帖子链接、发布时间、评论回流和下一步动作。
- 使用 `publish-record-template.json` 公开发布记录 JSON 结构。
- 使用 `feedback.html` 和 `feedback.js` 把发布记录 JSON 汇总成反馈问题榜。
- 使用 `feedback-records.json` 保存公开样例记录，真实发布后可以替换或追加。
- 使用 `faq.html` 和 `faq.js` 把高频反馈问题整理成公开答疑页。
- 使用 `reuse-map.json` 记录机器可读的复用链路。
- 使用 `start.html` 保存复用入口和关键材料链接。
- 使用 `reuse.html` 保存给读者扫读的复用路线。
- 使用 `monument.html` 记录项目为什么存在。
- 使用 `promo/xhs-post-drafts.md` 保存可直接复核和复制的小红书发布草稿。
- 使用 `promo/xhs-feedback-loop-template.md` 保存小红书发布后的反馈回流模板。
- 使用 `progress-timeline.json` 记录面向读者的进展时间线。
- 使用 `progress-timeline-source.json` 保存时间线源数据。
- 使用 `scripts/build-progress-timeline.mjs` 生成并检查进展时间线。
- 使用 `workflow.html` 展示 AI 会话公开化路线图。
- 使用 `trace-ledger.json` 记录作品如何一步步形成。
- 使用 `templates/ai-session-artifact-kit.md` 保存把 AI 会话公开化的复用模板。
- 使用 `templates/ai-session-public-quickstart.md` 保存短版公开化清单。
- 使用 `evidence-pack.md` 保存公开核验证据与复用流程。
- 使用 `verification-summary.md` 保存由公开 JSON 生成的验证摘要。
- 使用 `scripts/build-verification-summary.mjs` 生成并检查验证摘要。
- 使用 `scripts/scan-public-boundary.mjs` 扫描常见 token 前缀和本机绝对路径。
- 使用 GitHub Actions 在每次推送后自动运行回归验证。

## 下一步

- 继续把面向中文传播的说明、截图和发布素材整理好。
- 把新的设计取舍和验证证据持续同步到公开 Issue。
- 让进展时间线继续吸收 issue 里的有效记录。
- 保持作品轻量，不把它变成沉重的后台系统。

## 传播素材

- 小红书传播包：`promo/xhs-launch-kit.md`
- 小红书发布草稿包：`promo/xhs-post-drafts.md`
- 小红书发布后回流模板：`promo/xhs-feedback-loop-template.md`
- 小红书发布检查台：`promo/xhs-publish-checklist.md`
- 小红书发布 manifest：`promo/xhs-publish-manifest.json`
- 小红书发布前报告：`promo/xhs-publish-report.md`
- 小红书封面 PNG：`promo/xhs-cover.png`
- 小红书封面可编辑源：`promo/xhs-cover.html`
- 路线图长图 PNG：`promo/workflow-card.png`
- 路线图长图可编辑源：`promo/workflow-card.html`
- 复用流程图 PNG：`promo/reuse-flow-card.png`
- 复用流程图可编辑源：`promo/reuse-flow-card.html`
- 反馈问题榜长图 PNG：`promo/feedback-rank-poster.png`
- 反馈问题榜长图可编辑源：`promo/feedback-rank-poster.html`
- 网页分享卡片：`social-card.svg`
- 公开健康徽章：`public-health-badge.svg`
- 开始复用入口：`start.html`
- 复用路线：`reuse.html`
- 发布材料页：`launch.html`
- 公开材料总览：`materials.html`
- 材料选择器：`materials-guide.html`
- 发布前自检：`preflight.html`
- 反馈问题榜：`feedback.html`
- 公开 FAQ：`faq.html`
- 发布反馈样例记录：`feedback-records.json`
- 阅读了解包：`pack-read.html`
- 发帖发布包：`pack-publish.html`
- 复用接力包：`pack-reuse.html`
- 验证维护包：`pack-verify.html`
- 公开材料总览 JSON：`materials-index.json`
- 材料 API：`materials-api.json`
- 材料行动包：`materials-packs.json`
- 行动包分享卡：`promo/pack-publish-card.svg`
- 反馈问题榜分享卡：`promo/feedback-rank-card.svg`
- 反馈问题榜长图：`promo/feedback-rank-poster.png`
- 验证摘要：`verification-summary.md`
- 公开化路线图：`workflow.html`
- 进展时间线源数据：`progress-timeline-source.json`
- 项目纪念碑：`monument.html`
- 证据包：`evidence-pack.md`

## 复用模板

- AI 会话公开化模板：`templates/ai-session-artifact-kit.md`
- AI 会话公开化快速清单：`templates/ai-session-public-quickstart.md`

## 验证

```bash
npm run check
```

这个仓库没有运行时依赖。
